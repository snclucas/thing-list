...
@field_template.route(rule='/field-templates/<template_id>/sort', methods=['GET', 'POST'])
@login_required
def sort_template(template_id):
    if request.method == 'GET':
        ...
        return render_template(template_name_or_list='field_template/_sort_template_fields.html',
...
def template(template_id):
    ...
    :return: The rendered template or a 404 error message if the template
    ...
    if user_template_ is None:
        return render_template(template_name_or_list='404.html', message="No such template or you do not have "
                                                                         "access to this item"), 404
...
@field_template.route(rule='/set-template-fields', methods=['POST'])
@login_required
def set_template_fields():
    if request.method == 'POST':
        ...
        if request_xhr_key and request_xhr_key == 'XMLHttpRequest':
            ...
            if len(field_ids) == 0:
                abort(Response(response="At least 1 field is required for the template", status=400))
...
@field_template.route(rule='/field-templates/delete', methods=['POST'])
@login_required
def delete_template():
    json_data = request.json
...
@field_template.route(rule='/field-templates/add', methods=['POST'])
@login_required
def add_template():
    """
    The `add_template` method is used to add or update a field template.
    It is a route function that handles a POST request to '/field-templates/add'.
    ...
    if potential_template is None:
        template_, message = add_new_template(name=template_name,
                                              fields=template_fields, to_user=current_user)
        if template_ is None:
            flash("Problem adding template.")
            return redirect(url_for('field_template.templates'))
    else:
        status, message = update_template_by_id(template_data=new_template_data, user=current_user)
        if status is False:
            flash("Problem updating template.")
